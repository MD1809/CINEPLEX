  ...
  return (
    <aside className="w-72 bg-[#0b0e14] border-r border-slate-800/80 flex flex-col h-screen select-none p-4 shrink-0 shadow-2xl relative z-30 justify-between">
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        {/* Navigation Section */}
        <div className="px-2 pb-2 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest shrink-0">
          DANH MỤC QUẢN TRỊ
        </div>

        {/* Scrollable Nav Items */}
        <nav className="flex-1 overflow-y-auto space-y-1.5 pr-1 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
          {adminNavItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                to={item.to}
                className={({ isActive }) =>
                  `group flex items-center justify-between px-3 py-2.5 rounded-2xl transition-all duration-200 ${isActive
                ...
                {({ isActive }) => (
                  <>
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        className={`p-2 rounded-xl transition-colors shrink-0 ${isActive
                      ...
                      <div className="min-w-0">
                        ...
                        <p
                          className={`text-[10px] font-medium truncate mt-0.5 ${isActive ? 'text-slate-900/80' : 'text-slate-500'
                            }`}
                        >
                          {item.description}
                        </p>
                    ...
                    <ChevronRight
                      className={`w-3.5 h-3.5 transition-transform shrink-0 ${isActive
